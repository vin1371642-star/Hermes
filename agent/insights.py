...
class InsightsEngine:
    ...
    def format_terminal(self, report: Dict) -> str:
        ...
        if report["models"]:
            ...
            lines.append("  " + "─" * 56)
            lines.append(f"  {'Model':<30} {'Sessions':>8} {'Tokens':>12} {'Cost (USD)':>12}")
            ...
            for m in report["models"]:
                ...
                cost_str = f"${cost:.4f}" if has_pricing or cost > 0 else "?"
                if has_pricing or cost > 0:
                    total_known_cost += cost
                    any_cost_known = True
                lines.append(f"  {model_name:<30} {m['sessions']:>8} {m['total_tokens']:>12,} {cost_str:>12}")
            if any_cost_known and len(report["models"]) > 1:
                lines.append(f"  {'TOTAL':<30} {'':>8} {'':>12} ${total_known_cost:>10.4f}")
            lines.append("")

        # Cost summary (session-level totals from DB)
        overview_cost = report["overview"].get("estimated_cost", 0.0)
        if overview_cost and overview_cost > 0:
            lines.append("  💰 Estimated Spend")
            lines.append("  " + "─" * 56)
            lines.append(f"  Total (estimated):  ${overview_cost:.4f}")
            unknown_n = report["overview"].get("unknown_cost_sessions", 0)
            if unknown_n:
                lines.append(f"  Sessions with unknown pricing: {unknown_n}")
            lines.append("")
